...
// --- GÜVENLİK AYARLARI ---
var GLOBAL_API_KEY = "SD_SERVER_SECURE_KEY_2024"; // Bu anahtarı app.js ile aynı yapmalısınız!
...
var SHEET_HEADERS = {
  ...
  'Kullanıcılar': ['username', 'password_hash', 'role', 'full_name', 'token', 'force_password_change', 'two_fa_secret', 'two_fa_enabled'],
  'Ayarlar': ['type', 'value'],
...
function doPost(e) {
  try {
    ...
    if (action === 'addLog') {
      ...
      logSheet.appendRow([new Date(), payload.username, payload.action_type, payload.details, payload.ip]);
      return createJsonResponse({ status: 'success' });
    }

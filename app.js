...
async function deletePolicy(id) {
    ...
    try {
        const res = await fetch(gasUrl, {
            ...
            body: JSON.stringify({ action: 'deletePolicy', id: id, apiKey: sdApiKey })
        }).then(r => r.json());
    ...
    loginForm.addEventListener('submit', async(e)=>{
        ...
        try {
            ...
            if(res.status === "2fa_required") {
                ...
                handleLoginSuccess(res.user, rawP === "123456");
            } else {
                if(errBox) {
    ...
    twofaForm.addEventListener('submit', async(e) => {
        ...
        try {
            ...
            if(res.status === "success") {
                handleLoginSuccess(res.user, isDefaultPass);
            } else {
                if(errBox) {
...
function handleLoginSuccess(user, shouldForceChange) {
...
async function logAction(act, detail="") {
    ...
    try {
        fetch(gasUrl, {
            ...
            headers: { 'Content-Type': 'text/plain' },
            redirect: 'follow',
            body: JSON.stringify({ ...payload, apiKey: sdApiKey })
        });
    } catch(e) {
    ...
    formUser.addEventListener('submit', async (e) => {
        ...
        try { 
            const response = await fetch(gasUrl, { 
                ...
                headers: { 'Content-Type': 'text/plain' },
                redirect: 'follow',
                body: JSON.stringify({ action: "saveUser", apiKey: sdApiKey, data: data }) 
            }); 
            const res = await response.json();
...
function logout() {
    logAction('ÇIKIŞ', 'Kullanıcı oturumu kapattı').then(() => {
...
async function loadData() {
    try {
        ...
        const [pData, prData, clData, uData, lData, sData, finData] = await Promise.all([
            fetch(`${gasUrl}?action=getPolicies&apiKey=${sdApiKey}`).then(r=>r.json()),
            fetch(`${gasUrl}?action=getProposals&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[]),
            fetch(`${gasUrl}?action=getClaims&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[]),
            fetch(`${gasUrl}?action=getUsers&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[]),
            fetch(`${gasUrl}?action=getLogs&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[]),
            fetch(`${gasUrl}?action=getSettings&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[]),
            fetch(`${gasUrl}?action=getFinance&apiKey=${sdApiKey}`).then(r=>r.json()).catch(()=>[])
        ]);
...
document.getElementById('form-finance').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch(gasUrl, {
            method: 'POST',
            body: JSON.stringify({ action: 'saveFinance', apiKey: sdApiKey, data })
        }).then(r=>r.json());
...
async function addSetting(type) {
    ...
    try {
        await fetch(gasUrl, {
            method: 'POST',
            body: JSON.stringify({ action: 'saveSetting', type, value: val, apiKey: sdApiKey })
        });
        valInput.value = '';
        loadData();
...
async function deleteSetting(type, value) {
    ...
    try {
        await fetch(gasUrl, {
            method: 'POST',
            body: JSON.stringify({ action: 'deleteSetting', type, value, apiKey: sdApiKey })
        });
        loadData();
    } catch(e) { showToast("Hata","error"); }
}
...
async function approveProposal(id) {
    ...
    try {
        await fetch(gasUrl, {
            ...
            headers: { 'Content-Type': 'text/plain' },
            redirect: 'follow',
            body: JSON.stringify({ action: "saveProposal", apiKey: sdApiKey, data: p })
        });
    ...
    formPolicy.addEventListener('submit', async (e) => {
        ...
        try { 
            const response = await fetch(gasUrl, { 
                ...
                headers: { 'Content-Type': 'text/plain' },
                redirect: 'follow',
                body: JSON.stringify({ action: "savePolicy", apiKey: sdApiKey, data: data }) 
            }); 
            const res = await response.json();
